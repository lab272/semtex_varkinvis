...
%
Since it employs high-order finite element methods, \Semtex\ offers
the choice of element-size-based refinement (so-called $h$-refinement)
or polynomial-order-based refinement (so-called $p$-refinement) in
attempting to converge a solution (it is a $hp$-type method).  Since
the basis polynomials share convergence properties with (orthogonal)
Legendre polynomials, the underlying goal of mesh refinement is to
achieve exponential (`spectral') convergence of solutions with respect
to polynomial order; this typically commences when there are of order
$\pi$ polynomials per 'wavelength' of solution variation \citep{gs77}.

\Semtex\ tends to be most efficient at moderate polynomial orders (\eg
4 to 13); it is not a good idea to use either very low, or very high,
values of $p$.
%
...
$p=\texttt{N\_P}-1$.  On the other hand, carrying out $h$-refinement
will require a completely new session file to be produced, which may
imply quite a bit more work.  Consequently, the path of least
resistance is generally to produce a session file which one guesses is
quite well resolved as far as element sizes go, then rely on
converging the solution by starting on the low end of the $p$ range
and then increasing $p$ to higher, yet moderate, values in order to
commence exponential convergence.  Perhaps obviously, when tackling a
new problem, it pays to be prepared to carry out a few iterations of
mesh design.
%
