...
cheaper, it has become the default form in \Semtex\ (use
...
$2*\Delta t$.  It may also somewhat reduce the asymptotic convergence
...
your flow may have sensitive dynamics), it is generally best to use
either convective or full skew-symmetric forms.  However, in
computing, say, a turbulent flow, which is full of disturbance, you
might profitably choose the default alternating skew-symmetric form.
...
is, but the work of \citet{wikl01} showed that it is also linearly
unstable in a discrete setting.  And, in early developmental work for
our companion stability analysis code, \verb+dog+, it also proved to
produce erroneous stability analyses when the skew-symmetric form was
used.  So, it is not provided as an option in \verb+dns+ and indeed,
for this reason, the advective term used by \verb+dog+
($\bm{u}'\bm{\cdot\nabla U}+\bm{U\cdot\nabla u}'$) is based on the
convective form.
...
other forcing terms.  (It is usual in GFD Boussinesq applications to apply
...
previous section. In addition, one may need to apply frame
acceleration terms as outlined above in \S\S\,\ref{sec.constant},
\ref{sec.steady} and~\ref{sec.rotating}.
