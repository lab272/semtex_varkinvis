...
in regression testing (and in the following document).  Other files
...
More details are provided in the next chapter, but a brief outline is
that a session file has a number of sections that should appear once
each, but can be supplied in arbitrary order.  Any line whose first
character is \verb+#+ is a comment line.  Sections are demarcated (as
in HTML) using paired opening (\verb+<+name\verb+>+) and closing
(\verb+</+name\verb+>+) tags.  The opening tag will sometimes supply a
numeric parameter, \eg \verb+<NODES NUMBER=10>+.  The minimal set of
sections required for a session file to validly describe a domain in
which a solution can be obtained is: \verb+NODES+, \verb+ELEMENTS+ and
\verb+SURFACES+: \verb+NODES+ describe the corner vertices of
\verb+ELEMENTS+, while \verb+SURFACES+ describe what happens on the
outer boundaries of the domain. Sides of all \verb+ELEMENTS+ (which
run between \verb+NODES+) must either mate with the side of another
